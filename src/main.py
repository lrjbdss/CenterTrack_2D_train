...
def get_optimizer(opt, model):
    ...
    #     optimizer, milestones=opt.milestones, gamma=0.1)
    return optimizer
...
def main(opt):
    ...
    for epoch in range(start_epoch + 1, opt.num_epochs + 1):
        mark = epoch if opt.save_all else 'resnest_last'
        ...
        if opt.val_intervals > 0 and epoch % opt.val_intervals == 0:
            ...
            save_model(os.path.join(opt.save_dir, 'model_{}.pth'.format(opt.arch)),
                       epoch, model, optimizer)
        logger.write('\n')
        ...
        if epoch in opt.lr_step:
            lr = opt.lr * (0.2 ** (opt.lr_step.index(epoch) + 1))
            print('Drop LR to', lr)
